...
class VoxrayDAWState extends State<VoxrayDAW> with WidgetsBindingObserver {
  ...
  void initState() {
    ...
    positionTimer = Timer.periodic(const Duration(milliseconds: 33), (timer) {
      ...
      double currentT = 0.0;
      bool anyValid = false; // Track if any audio handle is actually alive
      ...
      if (masterHandle != null && SoLoud.instance.getIsValidVoiceHandle(masterHandle!)) {
        ...
        // Find the first valid stem handle to read the time from
        for (var handle in stemHandles.values) {
          if (SoLoud.instance.getIsValidVoiceHandle(handle)) {
            currentT = SoLoud.instance.getPosition(handle).inMilliseconds / 1000.0;
            anyValid = true;
            break;
          }
        }
      ...
      // If playing but all handles are dead, we reached the natural end of the song!
      if (!anyValid) {
        pauseAllPlayers();
  ...
  void setZoomY(double newZoom) {
    ...
    if (activeNotes.isNotEmpty) {
      ...
      anchorMidi = maxMidi - ((currentPixels + (viewportHeight / 2) - (oldZoom / 2)) / oldZoom);
    }
