        ...
        child: StatefulBuilder(
          builder: (context, setMixerState) {
            ...
            Widget buildChannelStrip(String title, String key, Color highlight, {bool isMaster = false}) {
              // 🟢 THE FIX: Wrap the entire strip in a StatefulBuilder to give it local UI updates
              return StatefulBuilder(
                builder: (context, setStripState) {
                  final state       = getChannelState(key);
                          ...
                          children: [
                            buildPluginSlot(key,state.plugin1, highlight, (val) {
                              if (state.plugin1 != val) {
                                setStripState(() { state.plugin1 = val!; }); // 🟢 Local UI redraw
                                setState(() { hasBeenSaved = false; });      // 🟢 Global DAW state
                            ...
                            buildPluginSlot(key,state.plugin2, highlight, (val) {
                              if (state.plugin2 != val) {
                                setStripState(() { state.plugin2 = val!; });
                            ...
                            buildPluginSlot(key,state.plugin3, highlight, (val) {
                              if (state.plugin3 != val) {
                                setStripState(() { state.plugin3 = val!; });
                                setState(() { hasBeenSaved = false; });
                                if (!isMaster) applyStemPlugins(key); else applyMasterPlugins();
                              }
                            }),
                            buildPluginSlot(key,state.plugin4, highlight, (val) {
                              if (state.plugin4 != val) {
                                setStripState(() { state.plugin4 = val!; });
                                setState(() { hasBeenSaved = false; });
                                if (!isMaster) applyStemPlugins(key); else applyMasterPlugins();
                              }
                            }),
                            ...
                            child: Row(
                              ...
                              children: [
                                InkWell(
                                  ...
                                  onTap: () {
                                    setStripState(() { state.isMuted = !state.isMuted; }); // 🟢 Local UI redraw
                                    setState(() { hasBeenSaved = false; });                // 🟢 Global DAW state
                                    refreshAllVolumes();
                                  },
                                  child: Padding(
                      ...
                      Expanded(
                        child: VoxrayHelpTarget(
                          ...
                          child: GestureDetector(
                            onDoubleTap: () {
                              setStripState(() { state.volume = 1.0; }); // Snap to 1.0 visually
                              setState(() { hasBeenSaved = false; });
                      ...
                      if (!isMaster) ...[
                        VoxrayHelpTarget(
                          ...
                          child: SizedBox(
                            ...
                            child: GestureDetector(
                              onDoubleTap: () {
                                setStripState(() { state.pan = 0.0; }); // Snap pan to center visually
                                setState(() { hasBeenSaved = false; });
                                if (key == 'original') { if (masterHandle != null) SoLoud.instance.setPan(masterHandle!, 0.0);
